...
var slider = (function($){
    function slider(selector,showDots){
        ...
        var _slide = function(i, duration) {
            ...
            if(index != i){
                if (i == 0) {
                    _addAnimationForFirstPage();
                }
                if (i == 1) {
                    _addAnimationForSecondPage();
                }
                if (i == 2) {
                    _removeAnimation();
                }
        ...
        var _addAnimationForFirstPage = function () {
            _removeAnimation();
            var firstPage = $('.J-page-1 img');
            firstPage.eq(0).addClass('flyPig');
            firstPage.eq(1).addClass('flyMap');
        };

        var _addAnimationForSecondPage = function () {
            _removeAnimation();
            var secondPage = $('.J-page-2 img');
            var css = '';
            for (var i = 0; i < 6; i ++) {
                css = 'circle-' + (i+1);
                secondPage.eq(i).addClass(css);
            }
        };

        var _removeAnimation = function () {
            var firstPage = $('.J-page-1 img');
            firstPage.eq(0).removeClass('flyPig');
            firstPage.eq(1).removeClass('flyMap');
            var secondPage = $('.J-page-2 img');
            var css = '';
            for (var i = 0; i < 6; i ++) {
                css = 'circle-' + (i+1);
                secondPage.eq(i).removeClass(css);
            }
        };
